...
class Advisor {
  ...
  public function scanDocument($parsedDoc): void {
    ...
    foreach ($parsedDoc->findAll('stanza:tag') as $k => $stanza) {
      ...
      if (empty($parsedTag)) {
        $this->add(Advice::createProblem('PROBLEM: Unparsable tag', $tagString));
      }
      else {
  ...
  protected function scanTag(string $tagString, \ParserGenerator\SyntaxTreeNode\Root $parsedTag): void {
    if ($parsedTag->findFirst('tag:condition')) {
      $this->add(Advice::createOK('OK', $tagString));
    }
    ...
    elseif ($parsedTag->findFirst('tag:block_open')) {
      ...
      $this->add(Advice::createProblem('PROBLEM: Unrecognized tag contents', $tagString));
    }
  }
  ...
  protected function scanExpressionTag(string $tagString, Branch $parsedTag) {
    ...
    if (str_contains($tagString, 'smarty:nodefaults')) {
      ...
      $this->add(Advice::createSuggestion('PROBLEM: In Smarty v5, "smarty:nodefaults" does not work. Use "nofilter".', $tagString, [$suggest]));
    ...
    if (str_starts_with($tagString, '{$form.')) {
      ...
      $this->add(Advice::createSuggestion('PROBLEM: This looks like an HTML widget. Specify "nofilter".', $tagString, [$this->appendNofilter($tagString)]));
    }
    elseif (str_ends_with($tagString, '|escape}') || str_ends_with($tagString, '|escape:"html"}')) {
      ...
      $this->add(Advice::createSuggestion('PROBLEM: This has specific escaping rules. Specify "nofilter" to ensure they are respected.', $tagString, [$this->appendNofilter($tagString)]));
    }
    else {
      // The data is ambiguous. It could be HTML widget... or an integer... or free-form text...
      $this->add(Advice::createSuggestion('PROBLEM: It is unclear if the variable has HTML-markup or plain-text. Choose unambiguous notation:', $tagString, [
